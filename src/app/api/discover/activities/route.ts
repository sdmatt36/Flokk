...
export async function GET(req: NextRequest) {
    ...
    rows = await db.$queryRaw(Prisma.sql`
      WITH raw_activities AS (
        SELECT
          ...
          COALESCE(ma.city, t."destinationCity") AS city,
          pr.rating,
          pr.notes AS "ratingNotes",
          pr."wouldReturn",
          ma.website AS "websiteUrl",
          NULL::text AS "sourceUrl",
          NULL::text AS "communitySpotWebsiteUrl",
        ...
        SELECT
          ...
          si."sourceUrl" AS "sourceUrl",
          cs."websiteUrl" AS "communitySpotWebsiteUrl",
          si."lat" AS "lat",
          si."lng" AS "lng",
        ...
        WHERE pr."savedItemId" IS NOT NULL
          AND (
            ...
            OR LOWER(COALESCE(si."destinationCity", '')) LIKE LOWER(${like})
          )
      ),
      aggregated AS (
        SELECT
          ...
          MAX("communitySpotWebsiteUrl") AS "communitySpotWebsiteUrl",
          MAX("lat") AS "lat",
          MAX("lng") AS "lng",
  ...
  const activities: DiscoverActivity[] = rows.map((r) => ({
    ...
    communitySpotWebsiteUrl: r.communitySpotWebsiteUrl ?? null,
